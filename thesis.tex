...
\thedivisionof{The Interdisciplinary Committee for Mathematics \& Biology}
...
% \doublespacing
	...
	Cell signaling networks are comprised of numerous complex interactions between many different proteins and protein complexes, and together comprise a cell's interactome.  Many of these interactions have been studied in great detail, and are illustrated by numerous manually-created images of cell signaling networks.  Recently, many of these figures have been converted into graphs, in which each node represents a unique protein, and each edge represents a known pairwise interaction between two proteins.  These graphs have made it possible for researchers to implement the multitude of known graph algorithms to try and find new areas of research that may yield novel or interesting results.  Unfortunately, the limited ways in which graphs allow protein-protein interactions to be represented lack biological realism, and struggle to represent complex interactions such as phosphorylation, negative regulation, and the existence of protein complexes. 

\section{Traditional Representations of Cell Signaling}



\section{Graphs of Cell Signaling Networks}
...
Given an input hypergraph $\mathcal{H}$, and a set of target nodes, $T$, we construct a Steiner Hypertree (define) using an ILP which encodes the definition of a Steiner Hypertree.  In order to accomplish this, we 

We find the Steiner Hypergraph $\mathcal{S} = \{x,e\}$ by optimizing the function:

\begin{equation} \label{eq:ilpsum}
 \max_{\alpha , \delta} \sum_{x \in V} g_x \alpha_x - \sum_{e \in E} c_e \alpha_e - \sum_{x \in V} h_x \delta_x
\end{equation}

Subject to:

\begin{equation} \label{eq:ilpT}
 \alpha_x \geq 1 \qquad \forall x \in T
\end{equation}

\begin{equation} \label{eq:ilpinchead}
 \sum_{x \in H_e} \alpha_x \leq \lvert H_e\rvert \alpha_e
\end{equation}

\begin{equation} \label{eq:ilpinctail}
 \sum_{x \in T_e} \alpha_x \leq \lvert T_e\rvert \alpha_e
\end{equation}

\begin{equation} \label{eq:ilpdang1}
 \delta_x \leq \alpha_x \qquad \forall x \in \mathcal{S}
\end{equation}

\begin{equation} \label{eq:ilpdang2}
 \delta_x \geq 1 - \sum_{e:x \in H_e} \alpha_e \qquad \forall x,e \in \mathcal{S}
\end{equation}

\begin{equation} \label{eq:ilpdang3}
 0 \leq \delta_x \leq 1 \qquad \forall x \in \mathcal{S}
\end{equation}
