...
One example of a cell signaling pathway that has a variety of biological consequences is the \textit{Hedgehog} (Hh) signal transduction network. Hedgehog is a protein that helps regulate limb formation during early development, cell development and differentiation, and the development of neural tubes (\cite{Hiu2011}). Hedgehog has been implicated in the development of basal cell carcinoma (BCC\footnote{Basal cell carcinoma is thought to be the most common form of cancer in humans.}) when it is overexpressed (\cite{Beachy2004}). Additionally, it has been shown that Hh has very powerful effects on the proper layout and development of tissues in mammals, and it has been proposed that Hh is responsible for assisting with stem cell assisted tissue regeneration in adult mammals (\emph{CITATION}).
...
\emph{This needs a good transition, so that it isn't so jarring to the reader.}
...
\begin{figure}[thbp]
  ...
  \caption[An example hypergraph]{An example of the hypergraph that corresponds to the standard graph shown in Figure \ref{fig:example-standard-graph}. Note that each edge now contains more specific information about the flow of information \textit{information?} through the network.}
...
\begin{figure}[!htbp]
  ...
  \begin{minipage}[b]{0.30\linewidth}
    ...
    \par\vspace{0pt}
  \end{minipage}
\caption[A small hypergraph, with node and edge data]{A simple hypergraph, and its associated node and edge weights. \emph{This obviously needs to be re-formatted to not look terrible.}}
\label{fig:dummy-before}
\end{figure}
...
\chapter{Implementation of PCSHT in PPI Data}
  ...
  \section{Simple Example Hypergraphs}
  ...
  \section{PCSHT in Real Data}
   ...
   \subsection{\textit{Arabidopsis thaliana} WGD Data}
